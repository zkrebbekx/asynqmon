...
// ****************************************************************************
// The scheduler lease-failure log is rate limited.
//
...
// drives the engine clock, so it never sleeps. It uses the package's fence
// DB 5 (see fence_test.go).
// ****************************************************************************
...
func TestSchedulerLeaseLogIsRateLimited(t *testing.T) {
	Convey("Given a hygiene engine whose Redis refuses the lease script", t, func() {
		fenceTestRedis(t)
		...
		Convey("When the lease tick runs 13 times over a minute", func() {
			...
			acquire := leaseLinesMatching(lines, "acquiring scheduler lease")

			Convey("Then the first line keeps its original wording", func() {
				So(len(acquire), ShouldBeGreaterThan, 0)
				So(acquire[0], ShouldEqual, "asynqmon: hygiene: acquiring scheduler lease: "+errNoPerm.Error())
			})
...
func TestSchedulerLeaseRenewLogUsesItsOwnKey(t *testing.T) {
	Convey("Given a hygiene engine that holds the scheduler lease", t, func() {
		fenceTestRedis(t)
		insp := asynq.NewInspector(asynq.RedisClientOpt{Addr: fenceTestRedisAddr, DB: fenceTestRedisDB})
		t.Cleanup(func() { insp.Close() })
		rc, deny := newNoPermClient(t)
		...
		Convey("When every command fails and two ticks run", func() {
			...
			Convey("Then the renewal line keeps its original wording", func() {
				renew := leaseLinesMatching(lines, "renewing scheduler lease")
				So(len(renew), ShouldEqual, 1)
				So(renew[0], ShouldEqual, "asynqmon: hygiene: renewing scheduler lease: "+errNoPerm.Error())
			})
